...
from common import GeneratingIterator, scan_from_file
...
class IteratorGenStrategy(Strategy):
    """ Strategy for filling iterables with random values """
    ...
    def execute(self, iterable: LinkedList):
        ...
        try:
            ...
            appended_list = LinkedList(*gen_iter)
            iterable.insert(index, *appended_list)
            Event("add", newList=appended_list, index=index, resultList=iterable)

            return True


class FileScanStrategy(Strategy):
    """ Strategy for scanning floats from file into iterable """
    ...
    def execute(self, iterable: LinkedList):
        ...
        try:
            ...
            appended_list = LinkedList(*scan_from_file(file_path, float))
            iterable.insert(index, *appended_list)
            Event("add", newList=appended_list, index=index, resultList=iterable)
            return True
...
class StrategiesManager:
    """ Class managing the strategies for iterable append"""
